...
int RGWRados::handle_object(librados::IoCtx &ioctx, struct ShardTracker &tracker,
                const string &name,
				      ...
				      check_filter_t force_check_filter)
{
  ...
  while (count < num_entries && !candidates.empty()) {
    ...
    r = handle_object(ioctx, tracker, name, dirent, m, bucket_info,
      updates, &count, last_entry_visited, force_check_filter);
    if (r < 0 && r != -ENOENT) {
      return r;
    }
    ...
    if (tracker.at_end() && tracker.is_truncated()) {
      ...
      for(auto iter = candidates.begin(); iter != citer; iter++) {
        ...
        ldpp_dout(dpp, 20) << "RGWRados::" << __func__ << " addtional key: "<< name << " added, shard " <<
            tracker.oid_name() << dendl;

        r = handle_object(ioctx, tracker, m, bucket_info,
          updates, &count, last_entry_visited, force_check_filter);
        if (r < 0 && r != -ENOENT) {
          return r;
        }
      ...
      ldpp_dout(cct, 20) << "RGWRados::" << __func__ << " this loop get "<< m.size() << " entries, shard " <<
        tracker.oid_name() << " consume to end, key: " << end_key << dendl;
